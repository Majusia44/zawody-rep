    ...
    <script>
        ...
        function generujFormularz(iloscPol) {
            ...
            for (let i = 1; i <= iloscPol; i++) { 
                ...
                formularz += '<input type="number" min=1 max=9 id="pole'+ i +'" name="pole'+ i +'" oninput="obliczSume()">';
                formularz += '<select id="operation' + i +'" name="operation"> <option value= "X">X</option><option value= 10>/</option><option value= 1>none</option></select><br>'
            }
